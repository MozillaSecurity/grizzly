...
class WebTransportH3Protocol(QuicConnectionProtocol):
    ...
    def _h3_event_received(self, event: H3Event) -> None:
        ...
        if (
            ...
            if self._http and not self._http.datagram_setting and len(event.data) > 0:
    ...
    def _receive_data_on_session_stream(self, data: bytes, fin: bool) -> None:
        ...
        for capsule in self._capsule_decoder_for_session_stream:
            ...
            if self._http.datagram_setting == H3DatagramSetting.RFC:
                self._receive_h3_datagram_rfc_capsule_data(capsule=capsule, fin=fin)
            elif self._http.datagram_setting == H3DatagramSetting.DRAFT04:
                self._receive_h3_datagram_draft04_capsule_data(capsule=capsule, fin=fin)

    def _receive_h3_datagram_rfc_capsule_data(
        self, capsule: H3Capsule, fin: bool
    ) -> None:
        if capsule.type == CapsuleType.DATAGRAM_RFC:
            ...
            self._set_close_info_and_may_close_session(data=capsule.data, fin=fin)
        else:
    ...
    def _receive_h3_datagram_draft04_capsule_data(
        ...
        if capsule.type == CapsuleType.REGISTER_DATAGRAM_NO_CONTEXT_DRAFT04:
            ...
            if format_type != WEBTRANPORT_FORMAT_TYPE:
                raise ProtocolError(
                    "Unexpected datagram format type: {}".format(format_type)
                )
            self._allow_datagrams = True
        elif capsule.type == CapsuleType.CLOSE_WEBTRANSPORT_SESSION:
            self._set_close_info_and_may_close_session(data=capsule.data, fin=fin)

    def _set_close_info_and_may_close_session(self, data: bytes, fin: bool) -> None:
    ...
    def _create_event_handler(
        self, session_id: int, path: bytes, request_headers: List[Tuple[bytes, bytes]]
    ) -> Any:
...
class WebTransportSession:
    ...
    def send_datagram(self, data: bytes) -> None:
        ...
        if not self._protocol._allow_datagrams:
            _logger.warn("Sending a datagram while that's now allowed - discarding it")
            return
...
class WebTransportEventHandler:
    ...
    def connect_received(self, response_headers: List[Tuple[bytes, bytes]]) -> None:
        self._run_callback(
            "connect_received", self._session.request_headers, response_headers
        )
