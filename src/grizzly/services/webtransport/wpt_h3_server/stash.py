...
StashManager.register("get_dict", callable=_get_shared, proxytype=DictProxy)
StashManager.register("Lock", callable=_get_lock, proxytype=AcquirerProxy)
...
StashManager.register("Queue", callable=queue.Queue, proxytype=QueueProxy)
...
class StashServer:
    ...
    def __enter__(self):
        self.manager, self.address, self.authkey = start_server(
            self.address, self.authkey, self.mp_context
        )
...
class Stash:
    ...
    def put(self, key, value, path=None):
        ...
        if internal_key in self.data:
            raise StashError(
                "Tried to overwrite existing shared stash value "
