...
def stream_reset(session, stream_id: int, error_code: int) -> None:
    ...
    data["stream-close-info"] = {"source": "reset", "code": error_code}
...
def session_closed(
    session, close_info: Optional[Tuple[int, bytes]], abruptly: bool
) -> None:
    token = session.dict_for_handlers["token"]
    data = session.stash.take(key=token) or {}
    ...
    if close_info:
        decoded_close_info = {"code": close_info[0], "reason": close_info[1].decode()}
