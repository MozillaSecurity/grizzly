...
def test_report_06(mocker, tmp_path):
...
def test_report_07(mocker, tmp_path, sig_cache, has_sig):
...
def test_report_08(mocker, backtrace, lines):
...
def test_report_09(tmp_path, data, lines):
...
def test_report_10(mocker, tmp_path, hang, has_log, expected):
...
def test_log_map_01(tmp_path):
    ...
    log_map = LogMap.create(tmp_path)
    assert all(log_map)
...
def test_log_map_02(tmp_path):
    """test minidump with LogMap.create()"""
    (tmp_path / "log_stderr.txt").write_bytes(b"STDERR log")
    (tmp_path / "log_stdout.txt").write_bytes(b"STDOUT log")
...
def test_log_map_03(tmp_path):
    """test selecting preferred DUMP_REQUESTED minidump with LogMap.create()"""
    (tmp_path / "log_stderr.txt").write_bytes(b"STDERR log")
    (tmp_path / "log_stdout.txt").write_bytes(b"STDOUT log")
    with (tmp_path / "log_minidump_01.txt").open("wb") as log_fp:
        log_fp.write(b"GPU|||\n")
...
def test_log_map_04(tmp_path):
    """test selecting worker logs with LogMap.create()"""
    (tmp_path / "log_stderr.txt").write_bytes(b"STDERR log")
    (tmp_path / "log_stdout.txt").write_bytes(b"STDOUT log")
...
def test_log_map_05(tmp_path):
    """test prioritizing sanitizer logs with LogMap._find_sanitizer()"""
    # NOTE: ordered by selection priority in order to use previously added logs
    # test empty
    (tmp_path / "log_asan.txt.0").touch()
    assert LogMap._find_sanitizer(list(tmp_path.iterdir())) is None

    # test *San log with data
    (tmp_path / "log_asan.txt.1").write_text("test")
    selected = LogMap._find_sanitizer(list(tmp_path.iterdir()))
    ...
    selected = LogMap._find_sanitizer(list(tsan_path.iterdir()))
    assert selected is not None
    assert selected == tsan_report


def test_log_map_06(tmp_path):
    """test selecting Valgrind logs with LogMap.create()"""
    (tmp_path / "log_stderr.txt").write_bytes(b"STDERR log")
    (tmp_path / "log_stdout.txt").write_bytes(b"STDOUT log")
