...
class LogMap:
    ...
    @classmethod
    def create(cls, path: Path) -> LogMap:
        """Scan path for file containing stderr, stdout and crashdata (aux)
        and build a LogMap.
        ...
        Returns:
            A LogMap pointing to log files.
        """
        ...
        log_err = None
        log_out = None
        for log_file in to_scan:
            if "stderr" in log_file.name:
                log_err = log_file
            elif "stdout" in log_file.name:
                log_out = log_file
        return cls(log_aux, log_err, log_out)
...
class Report:
    ...
    def __init__(
        ...
        self._logs = LogMap.create(log_path)
        # if both stderr and stdout are missing something is wrong
        assert self._logs.stderr or self._logs.stdout
    ...
    def calc_hash(signature: CrashSignature | None) -> str:
        ...
        Args:
            signature: Crash signature object or None.
    ...
    def tail(in_file: Path, size_limit: int) -> None:
        ...
        with in_file.open("rb") as in_fp:
            ...
            with open(out_fd, "wb") as out_fp:
                ...
                copyfileobj(in_fp, out_fp, 65536)
        in_file.unlink()
