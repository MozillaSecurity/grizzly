...
def test_interesting(tmp_path):
    ...
    with ReductionJob([], FakeTarget(), 30, False, False, 0, 1, 1, 0, 0) as obj:
...
def test_ignored(tmp_path):
    ...
    with ReductionJob(["foo", "bar"], MyTarget(), 30, False, False, 0, 1, 1, 0, 0) as obj:
        create_target_binary(obj.target, tmp_path)
        (tmp_path / "test.html").touch()
        obj.reduce_file = str(tmp_path / "test.html")
        obj.lithium_init()
...
def test_no_harness(tmp_path):
    ...
    with ReductionJob([], FakeTarget(), 30, True, False, 0, 1, 1, 0, 0) as obj:
        create_target_binary(obj.target, tmp_path)
        prefix = tmp_path / "lithium"
        prefix.mkdir()
...
def test_skip(tmp_path):
    ...
    with ReductionJob([], FakeTarget(), 30, False, False, 7, 1, 1, 0, 0,
                      testcase_cache=False) as obj:
        create_target_binary(obj.target, tmp_path)
        reduce_file = tmp_path / "test.html"
        reduce_file.touch()
        obj.reduce_file = str(reduce_file)
...
def test_any_crash_false(tmp_path):
    ...
    with ReductionJob([], MyTarget(), 30, False, False, 0, 1, 1, 0, 0,
                      testcase_cache=False) as obj:
        create_target_binary(obj.target, tmp_path)
        reduce_file = tmp_path / "test.html"
        reduce_file.touch()
...
def test_any_crash_true(tmp_path):
    ...
    with ReductionJob([], MyTarget(), 30, False, True, 0, 1, 1, 0, 0,
                      testcase_cache=False) as obj:
        create_target_binary(obj.target, tmp_path)
        reduce_file = tmp_path / "test.html"
        reduce_file.touch()
...
def test_min_crashes_repro(tmp_path):
    ...
    with ReductionJob([], FakeTarget(), 30, False, False, 0, 4, 1, 0, 0,
                      testcase_cache=False) as obj:
        create_target_binary(obj.target, tmp_path)
        reduce_file = tmp_path / "test.html"
        reduce_file.touch()
...
def test_min_crashes_norepro(tmp_path):
    ...
    with ReductionJob([], MyTarget(), 30, False, False, 0, 4, 1, 0, 0,
                      testcase_cache=False) as obj:
        create_target_binary(obj.target, tmp_path)
        reduce_file = tmp_path / "test.html"
        reduce_file.touch()
...
def test_repeat_repro(tmp_path):
    ...
    with ReductionJob([], FakeTarget(), 30, False, False, 0, 4, 17, 0, 0,
                      testcase_cache=False) as obj:
        create_target_binary(obj.target, tmp_path)
        reduce_file = tmp_path / "test.html"
        reduce_file.touch()
...
def test_repeat_norepro(tmp_path):
    ...
    with ReductionJob([], MyTarget(), 30, False, False, 0, 4, 17, 0, 0,
                      testcase_cache=False) as obj:
        create_target_binary(obj.target, tmp_path)
        reduce_file = tmp_path / "test.html"
        reduce_file.touch()
...
def test_no_cache(tmp_path):
    ...
    with ReductionJob([], FakeTarget(), 30, False, False, 0, 1, 1, 0, 0,
                      testcase_cache=False) as obj:
        create_target_binary(obj.target, tmp_path)
        (tmp_path / "test.html").touch()
        obj.reduce_file = str(tmp_path / "test.html")
...
def test_timeout_update(monkeypatch, tmp_path):
    ...
    with ReductionJob([], MyTarget(), 30, False, False, 0, 1, 1, 0, 30,
                      testcase_cache=False) as obj:
        create_target_binary(obj.target, tmp_path)
        (tmp_path / "test.html").touch()
        obj.reduce_file = str(tmp_path / "test.html")
...
def test_idle_timeout(mocker, tmp_path):
    ...
    with ReductionJob([], FakeTarget(), 30, False, False, 0, 1, 1, 25, 30,
                      testcase_cache=False) as obj:
        create_target_binary(obj.target, tmp_path)
        (tmp_path / "test.html").touch()
        obj.reduce_file = str(tmp_path / "test.html")
