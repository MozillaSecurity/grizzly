...
class CrashEntry:
    ...
    def testcases(self, subset=None):
        ...
        with NamedTemporaryFile(
            ...
            self._storage = Path(
                mkdtemp(prefix=f"crash-{self.crash_id}-", dir=grz_tmp("fuzzmanager"))
            )
            ...
            # directories would have: 'foo-2' (oldest), 'foo-1', 'foo-0' (most recent)
            # see FuzzManagerReporter for more info
            self._contents = sorted(
                (x.parent for x in self._storage.rglob("test_info.json")), reverse=True
            )

        if subset and self._contents:
            self._contents = self._subset(self._contents, subset)
